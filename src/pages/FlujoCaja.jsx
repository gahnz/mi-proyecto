...
const FlujoCaja = () => {
  ...
  // Efecto Debounce: Espera 500ms después de escribir para actualizar la búsqueda real
  useEffect(() => {
  ...
  // Efecto Reset Página: Vuelve a la página 1 si cambian los filtros
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, filterType, filterMonth]);

  // 2. Hook actualizado: Ahora recibe los filtros para enviarlos al servidor
  const {
    movements,
    stats,
    totalCount,
    ...
    deleteMovement,
    uploadDocument,
  } = useCashFlow(page, PAGE_SIZE, filterMonth, debouncedSearch, filterType);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const excelRows = fullData.map((m) => ({
        ...
        CATEGORIA:
          TAX_CATEGORIES.find((c) => c.id === m.category)?.label || m.category,
        DESCRIPCION: m.description,
        METODO_PAGO: m.payment_method,
        DOC_TIPO:
          DOCUMENT_TYPES.find((d) => d.id === m.doc_type)?.label || m.doc_type,
  ...
  const handleStockRestoration = async (itemsToRestore) => {
    ...
    try {
      ...
      toast.info("📦 Stock devuelto");
    } catch (error) {
      toast.error("Error al devolver stock");
    }
  };
  ...
  const handleReleaseFunds = async (movement) => {
    ...
    const promise = updateMovement(movement.id, {
      ...
      } (Liberado ${new Date().toLocaleDateString()})`,
    });
    toast.promise(promise, {
      loading: "Liberando...",
      success: "💰 Fondos disponibles",
      error: "Error",
    });
  };
  ...
  const handleSave = async () => {
    if (
      ...
      (!formData.receivedAmount || !formData.description)
    ) {
      toast.error("Faltan datos", {
        description: "Ingrese monto recibido y descripción.",
      });
      return;
    } else if (
      !formData.isEcommerce &&
      (!formData.totalAmount || !formData.description)
    ) {
      toast.error("Faltan datos", {
        description: "Ingrese monto y descripción.",
      });
      return;
    }
    ...
    const promise = (async () => {
      ...
      const isDigitalPayment = ["Mercado Pago"].includes(
        formData.paymentMethod
      );
      financialData.status =
        isEcommerce || isDigitalPayment ? "pending" : "confirmed";
      ...
      if (isEcommerce) {
        financialData = {
          ...
          taxAmount: 0,
          description: `${formData.description} (Venta: $${Number(
            formData.totalAmount || 0
          ).toLocaleString()} | Comis: $${Number(
      ...
      if (isEcommerce && formData.itemId && formData.warehouse && !editingId) {
        ...
        if (item) {
          const currentStock =
            item.stocksByWarehouse?.[formData.warehouse] || 0;
          if (currentStock >= formData.quantity) {
            ...
            throw new Error(`¡Stock insuficiente en ${formData.warehouse}!`);
        }
      }
      ...
      if (isEcommerce && formData.deliveryBy && !editingId) {
        ...
        if (deliveryError) {
          ...
          if (deliveryError.message?.includes("technician_paid")) {
            toast.warning(
              "Falta actualizar la base de datos (columna technician_paid)."
            );
          } else {
            toast.warning(
              `Error al asignar delivery: ${deliveryError.message}`
            );
          }
        } else {
          toast.success(
            `Delivery asignado a ${formData.deliveryBy} por $${DELIVERY_COST}`
          );
        }
      }
    })();
  ...
  const updateAmounts = (value, field) => {
    ...
    if (field === "total") total = parseFloat(value) || 0;
    else if (field === "received") received = parseFloat(value) || 0;
  ...
  // NOTA: Eliminamos filteredMovements porque el servidor ya devuelve los datos filtrados en 'movements'
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-20 w-full max-w-full overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 font-medium flex items-center gap-2">
            <Landmark size={14} className="text-brand-cyan" /> Control
            Financiero
          </p>
        </div>
        ...
        <div className="flex gap-3 w-full md:w-auto flex-wrap">
          ...
          >
            <ShoppingBag size={20} className="text-brand-cyan" /> Venta E-com
          </button>
          <button
            onClick={() => {
              resetForm();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Saldo Disponible (Real)"
          value={`$${Number(stats?.available_balance || 0).toLocaleString()}`}
        ...
        />
        <StatCard
          title="Saldo Retenido (En Tránsito)"
          value={`$${Number(stats?.pending_balance || 0).toLocaleString()}`}
        ...
        <StatCard
          title="IVA Neto F29"
          value={`$${Number(stats?.net_iva || 0).toLocaleString()}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-slate-900/40 border border-white/5 p-5 rounded-2xl flex items-center gap-4"
          >
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center ${b.bg} ${b.col}`}
            >
              {b.icon}
            </div>
            <div>
      ...
      <div className="bg-slate-900/50 backdrop-blur-xl p-4 rounded-2xl border border-white/5 flex flex-col md:flex-row gap-4 justify-between">
        <div className="flex gap-2 items-center">
          ...
          <div className="flex items-center gap-2 bg-slate-950/50 border border-white/10 rounded-xl px-3 py-1.5">
            ...
            <input
              type="month"
              className="bg-transparent border-none text-xs font-bold text-white outline-none uppercase"
              value={filterMonth}
              onChange={(e) => {
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder="Buscar MOV, ID, Cliente..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-slate-800/50 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-slate-200 outline-none focus:border-brand-purple/50"
          />
        </div>
      </div>
      ...
      <div className="bg-slate-900/50 rounded-2xl border border-white/5 overflow-hidden shadow-2xl">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {isPending ? (
                        <button
                          onClick={() => handleReleaseFunds(mov)}
                          className="flex items-center gap-1 bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2 py-1 rounded-lg text-[10px] font-bold uppercase hover:bg-amber-500/20 transition-all group/btn"
                          title="Clic para liberar"
                        >
                          <Lock size={10} className="group-hover/btn:hidden" />
                          <Unlock
                            size={10}
                      ...
                      <div className="flex flex-col">
                        ...
                        {mov.docUrl ? (
                          <a
                            ...
                            className="flex items-center gap-1 text-[10px] text-brand-cyan hover:text-white transition-colors w-fit"
                          >
                            <Eye size={12} /> Ver PDF
                          </a>
                        ) : (
                            ...
                            {uploadingId === mov.id ? (
                              ...
                              <label className="flex items-center gap-1 text-[10px] text-slate-500 hover:text-brand-cyan cursor-pointer w-fit transition-colors">
                                ...
                                <input
                                  ...
                                  onChange={(e) => handleFileUpload(e, mov.id)}
                                />
                              </label>
                            )}
                          </div>
                    ...
                    <td className="px-6 py-4 text-slate-300 text-sm max-w-xs">
                      ...
                      {mov.deliveryBy && (
                        <span className="block text-[9px] text-slate-500 mt-0.5 flex items-center gap-1">
                          <Truck size={10} /> {mov.deliveryBy}
                        </span>
                      )}
                      {mov.items && mov.items.length > 0 && (
                        <button
                          onClick={() => {
                            setViewingItems(mov.items);
                            setIsItemsModalOpen(true);
                          }}
                          className="mt-1 text-[10px] bg-brand-purple/10 text-brand-purple px-2 py-0.5 rounded border border-brand-purple/20 hover:bg-brand-purple/20 transition-all flex items-center gap-1 w-fit"
                        >
                          <ShoppingBag size={10} /> Ver {mov.items.length} ítems
                        </button>
                      )}
                    </td>

                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-slate-800 border border-white/5 text-slate-400">
                        {
                          TAX_CATEGORIES.find(
                            (c) => c.id === mov.category
                          )?.label?.split(" ")[0]
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-[10px] font-bold text-slate-300 uppercase px-2 py-1 bg-slate-800/50 rounded-lg border border-white/5">
                        {mov.paymentMethod}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      ...
                      >
                        {mov.type === "income" ? "+" : "-"} $
                        {Number(mov.totalAmount || 0).toLocaleString()}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right whitespace-nowrap">
        ...
        <div className="flex justify-between items-center bg-slate-950/50 p-4 border-t border-white/10">
          <button
            ...
            disabled={page === 1}
            className="p-3 bg-slate-800 rounded-xl hover:bg-slate-700 disabled:opacity-50 text-white border border-white/5"
          >
            <ChevronLeft size={20} />
          </button>
          <span className="text-xs font-bold text-slate-400">
            Pág <span className="text-white text-sm">{page}</span> /{" "}
            {totalPages || 1}
          </span>
          <button
            ...
            <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    {TAX_CATEGORIES.filter((c) => c.type === formData.type).map(
                      (c) => (
                ...
                <div className="grid grid-cols-3 gap-2">
                  {PAYMENT_METHODS.map((m) => (
                    <button
                      key={m}
                      onClick={() =>
                        setFormData({ ...formData, paymentMethod: m })
                      }
                      type="button"
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-white/5 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setTimeout(
                        () => updateAmounts(formData.totalAmount, "total"),
                        0
                      );
                    }}
                  />
                ...
                <div>
                  <label className="text-[10px] font-black text-brand-cyan uppercase block mb-1">
                    Monto Total
                  </label>
                  <input
                    type="number"
                    className="w-full bg-slate-900 border border-brand-cyan/30 rounded-xl p-3 text-white font-black text-xl outline-none"
                    value={formData.totalAmount}
                    onChange={(e) => updateAmounts(e.target.value, "total")}
                  />
                </div>
                <div className="flex gap-4 text-xs text-slate-500">
                  <span>Neto: ${formData.netAmount}</span>
                  <span>IVA: ${formData.taxAmount}</span>
                </div>
              </div>
              <button
                onClick={handleSave}
                className="w-full bg-brand-gradient py-4 rounded-xl text-white font-black uppercase tracking-widest shadow-lg hover:opacity-90"
              >
                Guardar Movimiento
              </button>
            </div>
            ...
            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-slate-900 sticky top-0 z-10">
              <div>
                ...
                <p className="text-[10px] text-slate-400">
                  Salida de Stock + Ingreso de Dinero (Retenido)
                </p>
              </div>
              <button onClick={() => setIsEcommerceModalOpen(false)}>
                <X className="text-slate-500 hover:text-white" />
              </button>
            </div>
            <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
              <div className="relative">
                <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 flex items-center gap-2">
                  <PackageMinus size={12} className="text-rose-400" /> 1.
                  Producto vendido (Salida Stock)
                </label>
                {selectedItemName ? (
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        setSelectedItemName("");
                        setItemSearchTerm("");
                        setFormData((p) => ({ ...p, itemId: "" }));
                      }}
                      className="bg-slate-800 p-3 rounded-xl text-slate-400 hover:text-white border border-white/10"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
                  ...
                  <>
                    <input
                      type="text"
                      className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-brand-cyan"
                            ...
                            <div
                              ...
                              onClick={() => {
                                setFormData((prev) => ({
                                  ...prev,
                                  itemId: item.id,
                                  description: `Venta Ecom: ${item.name}`,
                                  totalAmount: item.price_sell,
                                }));
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white outline-none text-center font-mono"
              ...
              <div className="bg-slate-950 p-5 rounded-2xl border border-white/5 relative overflow-hidden">
                ...
                <label className="text-[10px] font-bold text-slate-500 uppercase mb-3 flex items-center gap-2 relative z-10">
                  <DollarSign size={12} /> 2. Conciliación Financiera
                </label>
                <div className="space-y-3 relative z-10">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center w-40 bg-slate-900 border border-slate-700 rounded-lg px-2">
                      ...
                      <input
                        type="number"
                        className="w-full bg-transparent p-2 text-right text-white font-bold outline-none"
                        value={formData.totalAmount}
                        onChange={(e) => updateAmounts(e.target.value, "total")}
                      />
                    </div>
                  ...
                  <div className="flex justify-between items-center">
                    <label className="text-xs text-brand-cyan font-bold">
                      Monto Recibido (Banco)
                    </label>
                    <div className="flex items-center w-40 bg-slate-900 border border-brand-cyan/50 rounded-lg px-2 shadow-[0_0_10px_rgba(6,182,212,0.1)]">
                      ...
                      <input
                        type="number"
                        className="w-full bg-transparent p-2 text-right text-brand-cyan font-black outline-none"
                        value={formData.receivedAmount}
                        onChange={(e) =>
                          updateAmounts(e.target.value, "received")
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <label className="text-xs text-rose-400 font-bold">
                      Comisión / Costo Envío
                    </label>
                    <div className="text-right font-mono text-rose-400 font-bold">
                      - $
                      {Number(formData.commissionAmount || 0).toLocaleString()}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1 flex items-center gap-2">
                  <Truck size={12} /> Delivery realizado por
                </label>
                <select
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white outline-none"
                  value={formData.deliveryBy}
                  onChange={(e) =>
                    setFormData({ ...formData, deliveryBy: e.target.value })
                  }
                >
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-slate-950/50">
              <h3 className="font-bold text-white flex items-center gap-2">
                <ShoppingBag size={18} className="text-brand-cyan" /> Detalle de
                Venta
              </h3>
              <button
              ...
              <table className="w-full text-left text-xs">
                  ...
                  {viewingItems.map((item, idx) => (
                    <tr key={idx} className="text-slate-300">
                      ...
                      <td className="py-2 text-right">
                        $
                        {Number(
                          item.price || item.price_sell || 0
                        ).toLocaleString("es-CL")}
                      </td>
                      <td className="py-2 text-right font-bold text-brand-purple">
                        $
                        {(
                          Number(item.price || item.price_sell || 0) *
                          (item.quantity || 1)
                        ).toLocaleString("es-CL")}
                      </td>
                    </tr>
                ...
                <tfoot className="border-t border-white/10">
                  <tr>
                    <td
                      colSpan="3"
                      className="pt-3 text-right font-black text-slate-400 uppercase text-[10px]"
                    >
                      Total Calculado
                    </td>
            ...
            <div className="p-4 bg-slate-950/50 border-t border-white/10">
              <button
                onClick={() => setIsItemsModalOpen(false)}
                className="w-full bg-slate-800 hover:bg-slate-700 text-white py-2 rounded-xl font-bold text-xs transition-all"
              >
                Cerrar
              </button>
            </div>
  ...
  >
    <div
      className={`w-12 h-12 rounded-xl flex items-center justify-center ${
        isSpecial ? "bg-brand-purple text-white" : "bg-slate-800 text-slate-400"
